    ...
    <div class="viz-container">
        ...
        <div class="chart-card" *ngIf="symptomsData.length > 0">
          ...
          <div class="bar-chart">
            <div class="bar-row" *ngFor="let symptom of symptomsData; let i = index">
                  <div class="bar-label">{{ symptom }}</div>
                  <div class="bar" [style.width]="symptomsWidths[i] + '%'"></div>
                </div>
          </div>
        </div>
        ...
        <div class="chart-card" *ngIf="riskFactorsData.length > 0">
          ...
          <div class="risk-matrix">
            <div class="risk-item" *ngFor="let factor of riskFactorsData; let i = index">
              <div class="risk-label">{{ factor }}</div>
              <div class="risk-bar">
      ...
      <div class="comparison-section" *ngIf="symptomsData.length > 0 || riskFactorsData.length > 0">
        ...
        <div class="indicators">
          <div class="indicator" *ngFor="let ind of indicators">
            <div class="indicator-value">{{ ind.value }}%</div>
            <div class="indicator-label">{{ ind.label }}</div>
            <div class="indicator-bar">
              <div class="indicator-fill" [style.width]="ind.value + '%'"></div>
            </div>
          </div>
        </div>
      </div>
...
export class DataVisualizationComponent implements OnInit {
  ...
  ngOnInit() {
    this.symptomsData = this.symptoms.slice(0, 5);
    this.riskFactorsData = this.riskFactors.slice(0, 5);
    // Precompute random widths so values stay stable across change detection
    this.symptomsWidths = this.symptomsData.map(() => Math.random() * 80 + 20);
    this.riskWidths = this.riskFactorsData.map(() => Math.random() * 60 + 30);
  }
}
